...
| `BackupEncryptionService` | `data/services/backup_encryption_service.dart` | Key lifecycle: `enable(passphrase)` → generate random MLK + `libraryKeyId`, build passphrase and recovery keyslots, persist, return the recovery code; `unlock(secret)`; `disable()`; `changePassphrase(old,new)`; `regenerateRecoveryCode(currentSecret)`; `selfHealMirror()`. Fully independent of `SyncEncryptionService`. |
| `BackupEncryptionNotifier` + providers | `presentation/providers/backup_encryption_providers.dart` | Session key state (loaded/locked), gated on the prefs flag; exposes the active MLK to `BackupService`. Awaits `ensureLoaded()` before use (launch race), matching the #520 notifier pattern. |
...
| `BackupEncryptionSection` widget | `presentation/widgets/backup_encryption_section.dart` | The Settings UI (three states). |
