...
void main() {
  ...
  group('gas_compressibility', () {
    test('Z-factor is > 1 for air at high pressure', () {
      final z = gasCompressibilityFactor(o2Percent: 21, bar: 200);
      expect(z, greaterThan(1.0));
      expect(z, lessThan(1.1));
    });

    test('Z-factor is near 1 for low pressure', () {
      final z = gasCompressibilityFactor(o2Percent: 21, bar: 10);
      expect(z, closeTo(1.0, 0.01));
    });

    test('Z-factor differs for different gas mixes', () {
      final zAir = gasCompressibilityFactor(o2Percent: 21, bar: 200);
      final zTrimix = gasCompressibilityFactor(
        o2Percent: 21,
        hePercent: 35,
        bar: 200,
      );
      expect(zAir, isNot(closeTo(zTrimix, 0.001)));
    });

    test('extreme gas mix: Tx 10/70 at 300 bar has Z > 1 (He dominant)', () {
      // High-He mix: He has positive virial coefficients at moderate pressure,
      // pushing Z above 1 even more than air.
      final z = gasCompressibilityFactor(
        o2Percent: 10,
        hePercent: 70,
        bar: 300,
      );
      // He-dominant mix at 300 bar: Z should be notably > 1
      expect(z, greaterThan(1.05));
      expect(z, lessThan(1.20));
      // And different from air at same pressure
      final zAir = gasCompressibilityFactor(o2Percent: 21, bar: 300);
      expect(z, greaterThan(zAir));
    });

    test('gasVolume gives less than ideal gas at high pressure', () {
      final actual = gasVolume(
        tankSizeLiters: 12.0,
        pressureBar: 200,
        o2Percent: 21,
      );
      const ideal = 12.0 * 200; // ideal gas
      expect(actual, lessThan(ideal));
      // But should be close (within ~10%)
      expect(actual, greaterThan(ideal * 0.9));
    });

    test('gasVolume returns 0 for zero pressure', () {
      expect(gasVolume(tankSizeLiters: 12.0, pressureBar: 0, o2Percent: 21), 0);
    });

    test('barToAtm converts correctly', () {
      expect(barToAtm(1.01325), closeTo(1.0, 0.001));
      expect(barToAtm(2.0265), closeTo(2.0, 0.001));
    });
  });
}
